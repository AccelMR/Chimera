...
class Module
{
 public:
  ...
  instance() {
    if (!isStartedUp()) {
      CH_EXCEPT(InternalErrorException, "Trying to access a module but it hasn't been started.");
    }

    if (isDestroyed()) {

      CH_EXCEPT(InternalErrorException, "Trying to access a destroyed module.");
    }
  ...
  shutDown() {
    if (isDestroyed()) {
      CH_EXCEPT(InternalErrorException, "Trying to shut down an already shut down module.");
    }

    if (!isStartedUp()) {
      CH_EXCEPT(InternalErrorException, "Trying to shut down a module which was never started.");
    }
