...
ModelAsset::serialize(SPtr<DataStream> stream) {
  if (!stream) {
    CH_LOG(AssetSystem, Error, "Data stream is null for ModelAsset serialization");
    return false;
  }
  ...
  CH_LOG(AssetSystem, Debug, "ModelAsset {0} serialized successfully", m_metadata.name);
  return true;
}
